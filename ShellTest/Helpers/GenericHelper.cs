...
namespace ShellTest.Helpers
{
...

    {
        ...
        public static bool IsElemetPresent(By locator)
        {
            try
            {
                LogHelper.Write(" Checking for the element " + locator);
                return DriverContext.Driver.FindElements(locator).Count == 1;
            }
            catch (Exception)
            {
                return false;
        ...
        {
            var screen = DriverContext.Driver.TakeScreenshot();
            if (filename.Equals("Screen"))
            {
                filename = filename + DateTime.UtcNow.ToString("yyyy-MM-dd-mm-ss") + ".jpeg";
                screen.SaveAsFile(filename, ScreenshotImageFormat.Jpeg);
                LogHelper.Write(" ScreenShot Taken : " + filename);
                return;
            }
            screen.SaveAsFile(filename, ScreenshotImageFormat.Jpeg);
        ...
        {
            WebDriverWait wait = GetWebDriverWait(DriverContext.Driver, TimeSpan.FromSeconds(60));
        ...
        public static void WaitforElement( By locator, int wait)
        {
            ...
            WebDriverWait w = new WebDriverWait(DriverContext.Driver, TimeSpan.FromSeconds(wait));
        ...
        {
            WebDriverWait webDriverWait = new WebDriverWait(DriverContext.Driver, TimeSpan.FromSeconds(60));
        }
        ...
        public static void ExplicitWaitforElement(By locator, int wait)
        {
            ...
            try
            {
                ...
                WebDriverWait explicitwait = new WebDriverWait(DriverContext.Driver, TimeSpan.FromSeconds(wait));
